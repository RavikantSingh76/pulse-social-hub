...
export default function ReelsPage() {
  ...
  return (
    <div className="max-w-md mx-auto h-[calc(100vh-4.5rem)] relative pb-12 flex flex-col">
      {/* Top Segmented Mode Selector */}
      <div className="flex items-center justify-center p-2.5 z-40 bg-slate-950/60 backdrop-blur-md border-b border-slate-800/80">
        <div className="flex items-center p-1 bg-slate-900 rounded-2xl border border-slate-800">
          ...
          >
            ...
            <span>Community Feed ({reels.length})</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-scroll snap-y snap-mandatory scrollbar-none relative">
        {activeTab === 'featured' ? (
          <CinematicReelPlayer
            onOpenComments={(r) => setActiveCommentsPost(r)}
            onOpenShare={(r) => setActiveSharePost(r)}
          />
        ) : loading && reels.length === 0 ? (
          <div className="flex flex-col justify-center items-center h-[60vh] space-y-3">
            <div className="w-10 h-10 border-4 border-cyan-500 border-t-transparent rounded-full animate-spin"></div>
            <p className="text-xs text-slate-400 font-semibold tracking-wider uppercase">Loading Community Reels...</p>
          </div>
        ) : reels.length === 0 ? (
          ...
          reels.map((reel, index) => {
            ...
            return (
              <div
                key={reel.id}
                className="relative w-full h-[calc(100vh-8.5rem)] snap-start bg-slate-950 rounded-3xl overflow-hidden shadow-2xl mb-6 border border-slate-800 flex items-center justify-center select-none"
              >
                {/* Video Player */}
                {videoUrl ? (
                  <video
